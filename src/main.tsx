...
function Bulletin({
  ...
  const terms = Array.from({ length: school.terms }, (_, i) => i + 1),
    periods = Array.from({ length: school.periods_per_term }, (_, i) => i + 1);
  const slots = [...periods, 0];
  const assignments = data.assignments.filter(
    (a) => a.published && a.class_name === student.class_name,
  );
  const marks = data.marks.filter((m) => m.student_id === student.id);
  const cell = (subject: string, term: number, period: number) => {
    const aa = assignments.filter(
      (a) => a.subject_id === subject && a.term === term && a.period === period,
    );
    const mm = aa.map((a) => marks.find((m) => m.assignment_id === a.id));
    return {
      max: aa.reduce((n, a) => n + Number(a.max_score), 0),
      score: mm.reduce((n, m) => n + (m ? Number(m.score) : 0), 0),
      complete: aa.length > 0 && mm.every(Boolean),
    };
  };
  const all = data.subjects.flatMap((s) =>
    terms.flatMap((t) => slots.map((p) => cell(s.id, t, p))),
  );
  const max = all.reduce((n, c) => n + c.max, 0),
    score = all.reduce((n, c) => n + c.score, 0),
    complete = all.length > 0 && all.every((c) => c.complete);
  let lastDomain = "";
  return (
    <article className="bulletin">
      <div className="bulletin-heading">
        ...
        <div>
          ...
          <p>BULLETIN SCOLAIRE · MODÈLE DE DÉMONSTRATION</p>
        </div>
      ...
      <div className="bulletin-identity">
        ...
        <div>
          ...
          <p>
            NÉ(E) LE : {student.birth_date ? date(student.birth_date) : "—"}
          </p>
        </div>
      </div>
      <div className="table-scroll">
        <table className="report-table">
          <colgroup>
            <col style={{ width: "24%" }} />
            {Array.from(
              { length: 1 + terms.length * (slots.length + 1) },
              (_, i) => (
                <col key={i} />
              ),
            )}
          ...
          <thead>
            <tr>
              ...
              {terms.map((t) => (
                <th key={t} colSpan={slots.length + 1}>
                  TRIMESTRE {t}
                </th>
              ))}
              ...
              {terms.flatMap((t) => [
                ...slots.map((p) => (
                  <th key={t + "-" + p}>
                    {p === 0
                      ? "EXAMEN"
                      : "P" + ((t - 1) * school.periods_per_term + p)}
                  </th>
                )),
                <th key={t + "total"}>PTS / MAX</th>,
              ])}
            </tr>
          </thead>
          <tbody>
            {data.subjects.map((s) => {
              const domain = s.domain !== lastDomain;
              lastDomain = s.domain;
              const cc = terms.flatMap((t) =>
                slots.map((p) => cell(s.id, t, p)),
              );
              const sum = cc.reduce((n, c) => n + c.score, 0),
                maximum = cc.reduce((n, c) => n + c.max, 0);
              return (
                <ReactFragment key={s.id}>
                  {domain && (
                    <tr className="domain">
                      <th colSpan={2 + terms.length * (slots.length + 1)}>
                        {s.domain}
                      </th>
                    </tr>
                  )}
                  <tr>
                    <td>{s.name}</td>
                    {terms.flatMap((t) => {
                      const cc = slots.map((p) => cell(s.id, t, p));
                      return [
                        ...cc.map((c, i) => (
                          <td key={t + "-" + i}>
                            {c.complete ? c.score.toFixed(1) : "—"}
                            <small>/{c.max || "—"}</small>
                          </td>
                        )),
                        <td className="term-total" key={t + "sum"}>
                          {cc.every((c) => c.complete)
                            ? cc.reduce((n, c) => n + c.score, 0).toFixed(1)
                            : "—"}
                          <small>/{cc.reduce((n, c) => n + c.max, 0)}</small>
                        </td>,
                      ];
                    })}
                    <td className="term-total">
                      {cc.every((c) => c.complete) ? sum.toFixed(1) : "—"}
                      <small>/{maximum}</small>
                    </td>
                  </tr>
                </ReactFragment>
            ...
            <tr className="domain">
              <th>MAXIMA GÉNÉRAUX</th>
              {terms.flatMap((t) => [
                ...slots.map((p) => (
                  <td key={t + "-" + p}>
                    {data.subjects.reduce(
                      (n, s) => n + cell(s.id, t, p).max,
                      0,
                    )}
                  </td>
                )),
                <td key={t + "total"}>
                  {data.subjects.reduce(
                    (n, s) =>
                      n + slots.reduce((v, p) => v + cell(s.id, t, p).max, 0),
                    0,
                  )}
                </td>,
              ])}
      ...
      <div className="bulletin-result">
        <strong>
          POINTS OBTENUS : {complete ? score.toFixed(1) : "Incomplet"} / {max}
        </strong>
        <strong>
          POURCENTAGE :{" "}
          {complete && max ? ((score / max) * 100).toFixed(1) + " %" : "—"}
        </strong>
      </div>
      <p className="report-note">
        Seules les évaluations publiées sont reprises. « — » indique une
        évaluation manquante ou non notée. Ce document de démonstration n’est
        pas un bulletin officiel homologué.
      </p>
