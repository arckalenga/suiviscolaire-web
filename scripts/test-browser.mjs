...
try {
  ...
  await page.screenshot({
    path: ".local/screenshots/login.png",
    fullPage: true,
  });
  ...
  check(
    (await page.locator(".school-card").count()) === 3,
    "Main admin school directory",
  );
  await page.screenshot({
    path: ".local/screenshots/schools.png",
    fullPage: true,
  });
  await page
    .locator(".school-card")
  ...
  check(
    await page
      ...
      .first()
      .textContent()
      .then((t) => t.includes("12")),
    "School dashboard contains twelve students",
  );
  await page.screenshot({
  ...
  await page.screenshot({
    path: ".local/screenshots/bulletin.png",
    fullPage: true,
  });
  await page.pdf({
    path: ".local/screenshots/bulletin.pdf",
    format: "A4",
    landscape: false,
  ...
  check(
    (await page.locator(".subject-editor").count()) === 19,
    "Nineteen configurable branches",
  );
  await logout();
  ...
  check(
    ...
    "Single-school sub-admin interface",
  );
  await logout();
  ...
  check(
    (await page
      ...
      .count()) === 0,
    "Student has no administration navigation",
  );
  await page
  ...
  check(
    (await page.locator("tbody tr").count()) === 171,
    "Student assignment marks visible",
  );
  await page.getByRole("button", { name: "Paiements", exact: true }).click();
  await page
    .getByRole("heading", { name: "Historique des paiements" })
    .waitFor();
  check(
    (await page.locator("tbody tr").count()) === 3,
    "Student payment history visible",
  );
  await page
    .getByRole("button", { name: "Communications", exact: true })
    .click();
  check(
    (await page.locator(".message-list article").count()) === 2,
    "Student school communications visible",
  );
  await page
    .getByRole("button", { name: "Emploi du temps", exact: true })
    .click();
  check(
    (await page.locator(".day article").count()) === 20,
    "Student timetable visible",
  );
  await page
  ...
  await page.screenshot({
    path: ".local/screenshots/student-mobile.png",
    fullPage: true,
  });
  check(
    await page.evaluate(
